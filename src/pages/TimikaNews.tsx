...
export default function TimikaNews() {
  ...
  // Memisahkan data secara dinamis berdasarkan kategori atau urutan
  const featuredPosts = useMemo(() => TIMIKA_PULSE_POSTS.slice(0, 2), []); // 2 Berita teratas jadi Editor's Choice
  const latestPosts = useMemo(() => TIMIKA_PULSE_POSTS.slice(2), []);   // Sisanya masuk Fresh Updates

  useEffect(() => {
    // Simulasi loading cuaca
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#0a0a0a] text-zinc-900 dark:text-zinc-100 selection:bg-pink-500 selection:text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 py-12">
        ...
        <div className="lg:col-span-8 space-y-24">
          ...
          {/* Editor's Choice Section (Dinamis dari TIMIKA_PULSE_POSTS) */}
          <section className="space-y-12">
            ...
            <div className="space-y-20">
              {featuredPosts.map((post) => (
                <article key={post.slug} className="group grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                  <Link to={`/article/${post.slug}`} className="md:col-span-7 block overflow-hidden rounded-[2.5rem] bg-zinc-100 dark:bg-zinc-900 aspect-video relative shadow-2xl shadow-black/5">
                    ...
                    <div className="absolute top-6 left-6">
                      <span className="px-4 py-1.5 bg-white/90 dark:bg-black/80 backdrop-blur-md text-[9px] font-black uppercase rounded-full tracking-widest text-zinc-900 dark:text-white">
                        {post.category}
                      </span>
                    </div>
                  </Link>
          ...
          {/* Immersive Banner (Tetap Statis sebagai elemen desain) */}
          <div className="relative rounded-[3rem] overflow-hidden aspect-[16/9] lg:aspect-[21/9] group shadow-2xl">
            <img src="https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" alt="lorentz" />
            <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/20 to-transparent" />
            <div className="absolute inset-0 flex flex-col justify-center p-16 gap-6">
              <div className="flex items-center gap-2">
                <Globe size={12} className="text-pink-500 animate-spin-slow" />
                <span className="text-pink-400 text-[10px] font-black uppercase tracking-[0.3em]">Discovery Series</span>
              </div>
              <h4 className="text-5xl lg:text-6xl font-black text-white leading-none uppercase tracking-tighter max-w-xl">
                Lorentz: <br /> The Hidden <span className="text-pink-500">Emerald</span>
              </h4>
              <button className="w-fit px-8 py-4 bg-white text-black text-[10px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-pink-500 hover:text-white transition-all duration-500 shadow-xl">
                Explore Story
              </button>
            </div>
          </div>

          {/* Fresh Updates Section (Dinamis dari sisa TIMIKA_PULSE_POSTS) */}
          <section className="space-y-12">
