    ...
    <div className="relative min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 transition-colors duration-500 overflow-x-hidden font-sans selection:bg-pink-500/30">
      ...
      <header className="relative pt-32 md:pt-48 pb-12 max-w-5xl mx-auto px-6">
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
          ...
          <h1 className="text-4xl md:text-[90px] font-black leading-[0.95] tracking-tighter uppercase italic drop-shadow-sm">
            {post.title} <br />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pt-10 border-t border-zinc-100 dark:border-zinc-900">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden border border-zinc-200 dark:border-zinc-700 shadow-inner">
                <img src={`https://i.pravatar.cc/150?u=${post.author}`} alt={post.author} className="w-full h-full object-cover grayscale transition-all duration-500 hover:grayscale-0" />
              </div>
              <div className="text-left">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400 leading-none mb-1">Lead Editorial</p>
                <p className="text-sm font-bold hover:text-pink-500 cursor-pointer transition-colors">{post.author}</p>
              </div>
            </div>

            {/* Desktop Actions */}
            <div className="hidden md:flex items-center gap-3">
              <button className="p-4 rounded-full border border-zinc-100 dark:border-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-all active:scale-90 group shadow-sm"><Share2 size={18} /></button>
              <button className="p-4 rounded-full border border-zinc-100 dark:border-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-all active:scale-90 group shadow-sm"><Bookmark size={18} /></button>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-24 pb-32">
        ...
        <article className="lg:col-span-8">
          <div className="prose prose-zinc dark:prose-invert prose-lg max-w-none prose-headings:italic prose-headings:uppercase prose-headings:font-black prose-headings:tracking-tighter prose-blockquote:border-pink-500">
            ...
            <div className="not-prose bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 p-8 rounded-[2.5rem] mb-12 text-zinc-900 dark:text-white transition-colors">
          ...
          <div className="mt-16 pt-8 border-t border-zinc-100 dark:border-zinc-900">
            ...
            <div className="flex flex-wrap gap-3"> {/* gap-3 untuk jarak antar tag yang lebih pas */}
              {post.tags.map((tag) => (
                <Link to={`/tag/${tag.toLowerCase()}`} key={tag}>
                  <span className="px-5 py-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-900 text-[10px] font-bold uppercase tracking-widest text-zinc-600 dark:text-zinc-400 border border-zinc-100 dark:border-zinc-800 hover:border-pink-500 hover:text-pink-500 hover:shadow-md transition-all cursor-pointer active:scale-95 block">
                    {tag}
                  </span>
                </Link>
              ))}
        ...
        <aside className="lg:col-span-4 space-y-16">
          <div className="sticky top-32 space-y-16">
              ...
              <div className="space-y-10">
                {BLOG_POSTS.filter(p => p.slug !== slug).slice(0, 3).map((item) => (
                  <Link to={`/article/${item.slug}`} key={item.slug} className="group flex gap-5 items-start">
                    <div className="w-16 h-16 rounded-2xl bg-zinc-100 dark:bg-zinc-800 overflow-hidden shrink-0 shadow-sm border border-zinc-200 dark:border-zinc-800">
                    ...
                    <div className="space-y-1">
                      <h5 className="text-sm font-bold leading-tight group-hover:text-pink-500 transition-colors line-clamp-2">
            ...
            <div className="p-8 rounded-[2.5rem] bg-zinc-950 text-white relative overflow-hidden shadow-2xl">
                ...
                <p className="text-xs text-zinc-400 mb-6 leading-relaxed relative z-10">Dapatkan berita eksklusif Popverse langsung di inbox kamu.</p>
                <div className="flex flex-col gap-3 relative z-10">
                   ...
                   <button className="w-full bg-white text-zinc-950 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-pink-500 hover:text-white transition-all shadow-lg">Join The Tribe</button>
                </div>
            </div>
          </div>
        </aside>
      ...
      {/* --- MOBILE STICKY ACTIONS (Restored) --- */}
      <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] z-[70] animate-in slide-in-from-bottom-10 duration-700">
         <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 rounded-3xl p-4 flex items-center justify-around shadow-2xl text-zinc-900 dark:text-white ring-1 ring-black/5">
            <button className="flex flex-col items-center gap-1 group">
               <Heart size={20} className="group-active:scale-125 transition-transform" />
               <span className="text-[8px] font-black uppercase opacity-60">Like</span>
            </button>
            <div className="w-px h-8 bg-zinc-200 dark:bg-zinc-800" />
            <button className="flex flex-col items-center gap-1">
               <MessageCircle size={20} />
               <span className="text-[8px] font-black uppercase opacity-60">Chat</span>
            </button>
            <div className="w-px h-8 bg-zinc-200 dark:bg-zinc-800" />
            <button className="flex flex-col items-center gap-1">
               <Bookmark size={20} />
            ...
            <button className="flex flex-col items-center gap-1">
               <Share2 size={20} />
               <span className="text-[8px] font-black uppercase opacity-60">Share</span>
            </button>
         </div>
      </div>
      ...
      <footer className="py-24 text-center border-t border-zinc-100 dark:border-zinc-900 bg-white dark:bg-zinc-950 pb-32 lg:pb-24">
        <div className="flex flex-col items-center gap-12">
          <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="group flex flex-col items-center gap-4 transition-all active:scale-95">
      ...
      {/* --- REPAIR STYLE TAGS (Merged & Fixed) --- */}
      <style>{`
        ...
        /* FIX VIDEO RENDER (GPU Acceleration) */
        iframe {
