...
import { Sun, Moon, Menu, X, Search, Home as HomeIcon, Zap, ArrowLeft, Radio } from "lucide-react";
import { Link, useLocation } from "react-router-dom";
import useDarkMode from "../../hooks/useDarkMode";
import SearchOverlay from "../SearchOverlay";

export default function TimikaNavbar() {
  ...
  return (
    <>
      <nav
        ...
        className={`fixed top-0 z-[100] w-full transition-all duration-500 ease-in-out ${scrolled || open
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-12 items-center relative z-[110]">
            ...
            <div className="md:col-span-3 flex justify-start">
              <Link to="/timika/news" className="flex items-center gap-3 group active:scale-95 transition-transform" onClick={closeMenu}>
        ...
        {/* --- MOBILE MENU OVERLAY --- */}
        <div
          className={`fixed inset-0 h-[100dvh] w-screen md:hidden bg-white dark:bg-zinc-950 transition-all duration-500 ease-out ${open ? "translate-y-0 opacity-100 visible" : "-translate-y-full opacity-0 invisible"
            }`}
        >
          <div
